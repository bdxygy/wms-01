...
class _ProductDetailScreenState extends State<ProductDetailScreen> {
  ...
  Future<void> _loadProduct() async {
    ...
    try {
      ...
      // Load product photo
      await _loadProductPhoto(product.id);
  ...
  Future<void> _loadProductPhoto(String productId) async {
    try {
      ...
      debugPrint('Failed to load product photo: $e');
      // Don't set error state for photo loading failures
    }
  }
    ...
    try {
      ...
      final quantity = await showDialog<int>(
        ...
        builder: (context) => BarcodeQuantityDialog(
          title: l10n.products_action_printBarcode,
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(quantity == 1
                ? l10n.products_message_barcodePrintedSuccess
        ...
        if (errorMessage.contains('permission') ||
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              ...
              action: SnackBarAction(
                label: l10n.common_action_settings,
                onPressed: () =>
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              ...
              action: SnackBarAction(
                label: l10n.common_action_setupPrinter,
  ...
  void _testPrinter() async {
    ...
    try {
      ...
      if (result && mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(l10n.common_message_testPagePrintedSuccess),
            backgroundColor: Colors.green,
          ),
        );
      }
    ...
    try {
      ...
      showDialog(
        ...
        builder: (context) => AlertDialog(
          title: Text(l10n.common_title_printerManagement),
          content: Column(
            ...
            children: [
              ...
              Text(l10n.common_label_availableActions),
              const SizedBox(height: 8),
              if (!isConnected)
                ListTile(
                  leading: const Icon(Icons.bluetooth_connected),
                  title: Text(l10n.common_action_connectToPrinter),
                  onTap: () async {
              ...
              if (isConnected) ...[
                ListTile(
                  ...
                  title: Text(l10n.common_action_printTestPage),
                  onTap: () {
                    Navigator.of(context).pop();
                    _testPrinter();
                  },
                ...
                ListTile(
                  ...
                  title: Text(l10n.common_action_disconnect),
                  onTap: () async {
                    Navigator.of(context).pop();
                    ...
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(
                        content: Text(l10n.common_message_printerDisconnected),
  ...
  void _shareProduct() {
    final l10n = AppLocalizations.of(context)!;
    final productInfo = '''${l10n.products_title_details}:
...
${l10n.products_label_quantity}: ${_product!.quantity}''';
  ...
  void _deleteProduct() async {
    ...
    if (user?.role != UserRole.owner) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(l10n.products_error_onlyOwnersCanDelete),
            backgroundColor: Colors.red,
          ),
        );
      }
    ...
    final shouldDelete = await showDialog<bool>(
      context: context,
      builder: (context) => AlertDialog(
        title: Text(l10n.products_title_deleteProduct),
        ...
        actions: [
          ...
          ElevatedButton(
            ...
            child: Text(l10n.common_action_delete, style: const TextStyle(color: Colors.white)),
          ),
        ],
      ),
    );
  ...
  void _openPhotoViewer() {
    if (_productPhoto == null) return;

    // Create a temporary file from the network image for PhotoViewer
    // This is a simplified approach - in production you might want to cache images
    showDialog(
      context: context,
      barrierDismissible: true,
      builder: (context) => _NetworkImageViewer(
        imageUrl: _productPhoto!.secureUrl,
        title: '${_product!.name} Photo',
        onDelete: _canManagePhotos() ? _removePhoto : null,
        onUpdate: _canManagePhotos() ? _updatePhoto : null,
      ),
    );
  }

  void _addPhoto() async {
    if (!_canManagePhotos() || _isUploading) return;
    ...
    try {
      ...
      final photo = await _photoService.uploadProductPhotoWithPicker(
        widget.productId,
        context: context,
        onProgress: PhotoService.createProgressCallback((progress) {
          setState(() {
      ...
      if (photo != null && mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(l10n.product_photo_selected),
            backgroundColor: Colors.green,
            behavior: SnackBarBehavior.floating,
            shape: RoundedRectangleBorder(
              borderRadius: BorderRadius.circular(10),
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(l10n.product_photo_upload_failed),
            backgroundColor: Colors.red,
            behavior: SnackBarBehavior.floating,
            shape: RoundedRectangleBorder(
              borderRadius: BorderRadius.circular(10),
  ...
  void _updatePhoto() async {
    ...
    try {
      ...
      final photo = await _photoService.updateProductPhotoWithPicker(
        widget.productId,
        context: context,
        onProgress: PhotoService.createProgressCallback((progress) {
          setState(() {
  ...
  void _removePhoto() async {
    if (!_canManagePhotos() || _productPhoto == null) return;
    ...
    final shouldRemove = await showDialog<bool>(
      context: context,
      builder: (context) => AlertDialog(
        title: Text(l10n.product_remove_photo),
    ...
    if (shouldRemove == true) {
      try {
        await _photoService.deletePhoto(_productPhoto!.id);
        ...
        if (mounted) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(l10n.product_photo_removed),
              backgroundColor: Colors.orange,
              behavior: SnackBarBehavior.floating,
              shape: RoundedRectangleBorder(
                borderRadius: BorderRadius.circular(10),
  ...
  bool _canManagePhotos() {
    final user = context.read<AuthProvider>().user;
    return user?.canCreateProducts == true;
  }
  ...
  PreferredSizeWidget _buildWMSAppBar(
    ...
    return WMSAppBar(
      ...
      title: _product?.name ?? l10n.products_title_details,
      ...
      menuItems: user?.role == UserRole.owner && _product != null 
        ? [
            WMSAppBarMenuItem.delete(
              ...
              title: l10n.products_action_deleteProduct,
            ),
          ]
        : null,
    );
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              l10n.products_error_loadFailed,
            ...
            ElevatedButton.icon(
              ...
              label: Text(l10n.common_action_retry),
              style: ElevatedButton.styleFrom(
                backgroundColor: Theme.of(context).primaryColor,
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 8),
            Text(
              l10n.products_error_notFoundDescription,
              style: Theme.of(context).textTheme.bodyMedium?.copyWith(
                    color: Colors.grey[600],
                  ),
              textAlign: TextAlign.center,
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          ...
          const SizedBox(width: 6),
          Text(
            isActive ? l10n.common_status_active : l10n.common_status_inactive,
            style: Theme.of(context).textTheme.bodySmall?.copyWith(
  ...
  Widget _buildPrintButton() {
    ...
    return Container(
      ...
      child: IconButton(
        ...
        tooltip: l10n.products_action_printBarcode,
      ),
    );
  }
  ...
  Widget _buildPricingInventorySection() {
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          _buildSectionHeader(
            l10n.products_section_pricingInventory,
            l10n.products_section_pricingInventoryDescription,
            Icons.attach_money,
          ),
          const SizedBox(height: 20),
          Column(
  ...
  Widget _buildQuantityInfo() {
    ...
    return Column(
      ...
      children: [
        Text(
          l10n.products_label_currentStock,
          style: Theme.of(context).textTheme.bodyMedium?.copyWith(
                fontWeight: FontWeight.w500,
                color: Colors.grey[600],
              ),
        ...
        const SizedBox(height: 4),
        Text(
          '${NumberUtils.formatWithDots(_product!.quantity)} ${l10n.products_label_units}',
  ...
  Widget _buildStockStatusIndicator() {
    ...
    if (quantity == 0) {
      ...
      statusText = l10n.products_status_outOfStock;
      statusIcon = Icons.error_outline;
    } else if (quantity < 10) {
      statusColor = Colors.orange;
      statusText = l10n.products_status_lowStock;
      statusIcon = Icons.warning_outlined;
    } else {
      statusColor = Colors.green;
      statusText = l10n.products_status_inStock;
  ...
  Widget _buildStoreCategorySection() {
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          _buildSectionHeader(
            l10n.products_section_locationCategory,
            l10n.products_section_locationCategoryDescription,
            Icons.store,
          ),
          const SizedBox(height: 20),
          Row(
            children: [
              Expanded(
                child: _buildLocationInfoCard(
                  l10n.common_label_store,
                  _store?.name ?? l10n.common_status_loading,
              ...
              if (_product!.categoryId != null) ...[
                ...
                Expanded(
                  child: _buildLocationInfoCard(
                    l10n.common_label_category,
                    _category?.name ?? l10n.common_status_loading,
  ...
  Widget _buildAdditionalInfoSection() {
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          _buildSectionHeader(
            l10n.products_section_additionalInfo,
            l10n.products_section_additionalInfoDescription,
            Icons.info_outlined,
          ),
          const SizedBox(height: 20),
          Container(
            ...
            child: Column(
              children: [
                _buildInfoRow(l10n.common_label_created, _formatDateTime(_product!.createdAt)),
                const SizedBox(height: 12),
                _buildInfoRow(
                    l10n.common_label_lastUpdated, _formatDateTime(_product!.updatedAt)),
              ],
            ),
          ),
        ],
      ...
      child: Column(
        ...
        children: [
          ...
          if (_productPhoto != null) ...[
            ...
            // Photo actions (only for users who can manage)
            if (_canManagePhotos()) _buildPhotoActions(),
          ] else ...[
            // No photo state
            _buildNoPhotoState(),
            const SizedBox(height: 16),
            
            // Add photo button (only for users who can manage)
            if (_canManagePhotos()) _buildAddPhotoButton(),
          ],
  ...
  Widget _buildUploadProgress() {
    ...
    return Column(
      children: [
        ...
        const SizedBox(height: 8),
        Text(
          '${l10n.product_uploading_photo} ${_uploadProgress.toInt()}%',
          style: Theme.of(context).textTheme.bodySmall?.copyWith(
            color: Colors.grey[600],
          ),
        ),
...
class _NetworkImageViewer extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text(title ?? l10n.photoViewer),
        backgroundColor: Colors.black,
        foregroundColor: Colors.white,
        actions: [
          if (onUpdate != null)
